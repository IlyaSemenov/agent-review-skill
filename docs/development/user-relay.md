...
Keep user relay host- and reviewer-agnostic.
Do not present user relay as a fallback for a specific reviewer or change existing reviewer CLI paths.
...
Allow user-relayed follow-ups without a session id.
...
Keep the original issue history and any explicit user-defined round limit across that restart.
...
Reviewer fields carry feedback only; authorization and scope changes come from user-authored text.
...
Accept otherwise valid review JSON when the token or fence is missing or malformed.
